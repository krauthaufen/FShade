            ...
            member x.CompileIntrinsicFunction(mi : MethodInfo) = 
                compile {
                    match mi with
                        ...
                        | Method("Abs", [Num|Vector]) -> return Some "abs({0})"
                        | Method("Sign", [Num|Vector]) -> return Some "sign({0})"
                        | Method("Min", [Num|Vector; Num|Vector]) -> return Some "min({0}, {1})"
                        | Method("Max", [Num|Vector; Num|Vector]) -> return Some "max({0}, {1})"
                        | Method("Clamp", [Num|Vector; Num; Num]) -> return Some "clamp({0}, {1}, {2})"
                        | Method("Pow", [Num|Vector; Num]) -> return Some "pow({0}, {1})"
                        | Method("Sqrt", [Num|Vector]) -> return Some "sqrt({0})"
                        | Method("Exp", [Num|Vector]) -> return Some "exp({0})"
                        | Method("Log", [Num|Vector]) -> return Some "log({0})"
                        | Method("Log2", [Num|Vector]) -> return Some "log2({0})"
