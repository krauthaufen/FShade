    ...
    module Helpers = 
        ...
        let rec tryGetBuiltInMethod (b : IBackend) (mi : MethodInfo) (args : list<CExpr>) =
            ...
            match mi, args with
                ...
                | ColMethod("AnyEqual", _), [x; y]
                | VecMethod("AnyEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAnyEqual(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAnyEqual(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAnyEqual(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allEqual : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllEqual", _), [x; y]
                | VecMethod("AllEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CEqual(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CEqual(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CEqual(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.anyDifferent : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AnyDifferent", _), [x; y]
                | VecMethod("AnyDifferent", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CNotEqual(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CNotEqual(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CNotEqual(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allDifferent : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllDifferent", _), [x; y]
                | VecMethod("AllDifferent", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAllNotEqual(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAllNotEqual(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAllNotEqual(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.anySmaller : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AnySmaller", _), [x; y]
                | VecMethod("AnySmaller", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAnyLess(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAnyLess(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAnyLess(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allSmaller : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllSmaller", _), [x; y]
                | VecMethod("AllSmaller", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAllLess(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAllLess(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAllLess(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.anySmallerOrEqual : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AnySmallerOrEqual", _), [x; y]
                | VecMethod("AnySmallerOrEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAnyLequal(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAnyLequal(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAnyLequal(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allSmallerOrEqual : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllSmallerOrEqual", _), [x; y]
                | VecMethod("AllSmallerOrEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAllLequal(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAllLequal(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAllLequal(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.anyGreater : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AnyGreater", _), [x; y]
                | VecMethod("AnyGreater", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAnyGreater(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAnyGreater(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAnyGreater(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allGreater : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllGreater", _), [x; y]
                | VecMethod("AllGreater", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAllGreater(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAllGreater(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAllGreater(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.anyGreaterOrEqual : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AnyGreaterOrEqual", _), [x; y]
                | VecMethod("AnyGreaterOrEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAnyGequal(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAnyGequal(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAnyGequal(CNewVector(vt, [x]), y) |> Some
                    | _ -> None

                | MethodQuote <@ Vec.allGreaterOrEqual : V2d -> V2d -> bool @> _, [x; y]
                | ColMethod("AllGreaterOrEqual", _), [x; y]
                | VecMethod("AllGreaterOrEqual", _), [x; y] ->
                    match x.ctype, y.ctype with
                    | (CVector _ | CColor _), (CVector _ | CColor _) -> CVecAllGequal(x, y) |> Some
                    | (CVector _ | CColor _ as vt), _                -> CVecAllGequal(x, CNewVector(vt, [y])) |> Some
                    | _, (CVector _ | CColor _ as vt)                -> CVecAllGequal(CNewVector(vt, [x]), y) |> Some
                    | _ -> None
    ...
    and toCExprS (e : Expr) : State<_, CExpr> =
        state {
            ...
            match e with
                ...
                | RefOf v ->
                    match! toCLExprS v with
                    | Some lv -> return CLExpr.toExpr lv
                    | None -> return failwith $"[FShade] cannot get address of non L-Expression: {v}"
