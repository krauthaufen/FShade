...
module PrimitiveType =
    ...
    let rec ofShaderType (t : ShaderType) =
        match t with
            ...
            | _ -> failwithf "cannot convert type %A to PrimitiveType" t 
    ...
    let rec toUniformType (t : ShaderType) : UniformType =
        match t with
            ...
            | ShaderType.Image(sampledType, dim,depth,arr, ms, sam, fmt) -> 
                failwithf "cannot determine size for image type"

            | ShaderType.Sampler -> 
                failwithf "cannot determine size for sampler type"


            | ShaderType.Void -> 
                failwithf "cannot determine size for void type"

            | ShaderType.Function _ ->
                failwithf "cannot use function in UniformBuffer"

            | ShaderType.SampledImage _ ->
                failwithf "cannot use SampledImage in UniformBuffer"
    ...
    let ofShaderParameter (p : ShaderParameter) =
        let location =
            match ShaderParameter.tryGetLocation p with
                | Some loc -> loc
                | None -> failwithf "could not get input location for parameter %A" p
        ...
        let paramType, arraySize =
            match p.paramType with
                ...
                | _ ->
                    failwithf "input parameter %A has invalid type %A" p.paramName p.paramType
...
module ShaderUniformParameter =
    ...
    module Dim =
        let toTextureDimension =
            LookupTable.lookup [
                ...
                Dim.Cube, SamplerDimension.SamplerCube
                Dim.Rect, SamplerDimension.Sampler2d
                Dim.Buffer, SamplerDimension.Sampler1d
                Dim.SubpassData, SamplerDimension.Sampler2d
            ]
    ...
    let ofShaderParameter (p : ShaderParameter) =
        match p.paramType with
            | ShaderType.Ptr((StorageClass.Uniform | StorageClass.UniformConstant), realType) ->
                let set =
                    match ShaderParameter.tryGetDescriptorSet p with
                        | Some set -> set
                        | None -> failwithf "uniform %A does not provide a DescriptorSet" p.paramName
                        
                let binding =
                    match ShaderParameter.tryGetBinding p with
                        | Some set -> set
                        | None -> failwithf "uniform %A does not provide a binding" p.paramName
            ...
            | t ->
                failwithf "uniform %A has unexpected type %A" p.paramName t
                ...
                | OpVariable(typeId, id, kind, _) ->
                    match kind with
                        ...
                        | StorageClass.PushConstant ->
                            failwithf "PushConstants not supported atm."
            ...
            |> Seq.map (fun m ->
                let mutable stage, kind =
                    match m.entryModel with
                        ...
                        | m -> failwithf "unsupported ExecutionModel %A" m
